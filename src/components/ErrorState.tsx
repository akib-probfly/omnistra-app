...
export function ErrorState({ message = 'Something went wrong. Please try again.', onRetry }: { message?: string; onRetry?: () => void }) {
  return <View style={styles.wrap}><View style={styles.icon}><AlertCircle color="#dc2626" size={24} /></View><Text style={styles.title}>Unable to load</Text><Text style={styles.message}>{message}</Text>{onRetry ? <Pressable onPress={onRetry} style={styles.retry}><RefreshCw color="#fff" size={16} /><Text style={styles.retryText}>Try again</Text></Pressable> : null}</View>;
}
